...
export async function POST(req: Request) {
  ...
  if (ctx.data.cashflow.length === 0) {
    ...
    insights.push({ titulo: "Fluxo de caixa positivo", severidade: "baixa", detalhe: `Saldo do período: R$ ${saldo.toFixed(2)}.` });
  }

  const ativos = ctx.data.costs.filter((c:any)=>String(c.status||"").toLowerCase()==="active").length;
  if (ativos === 0) {
    insights.push({ titulo: "Nenhum custo fixo ativo cadastrado", severidade: "media", detalhe: "Cadastre custos para projeção mensal confiável." });
  } else {
    insights.push({ titulo: `${ativos} custo(s) fixo(s) ativo(s)`, severidade: "baixa", detalhe: "Verifique reajustes e fornecedores periodicamente." });
  }

  if (ctx.data.loans.length > 0) {
    const parc = ctx.data.loans.reduce((s:number,l:any)=>s+Number(l.current_installment_amount||0),0);
    insights.push({ titulo: `${ctx.data.loans.length} empréstimo(s) cadastrado(s)`, severidade: parc>saldo?"alta":"media",
      detalhe: `Parcela mensal total: R$ ${parc.toFixed(2)}.` });
  } else {
    insights.push({ titulo: "Sem empréstimos cadastrados", severidade: "baixa", detalhe: "Cadastre contratos para projetar parcelas e saldo devedor." });
  }
  ...
  if (ctx.data.dre.length > 0) {
    ...
    insights.push({ titulo: "DRE preenchida no período", severidade: "baixa", detalhe: `Receitas R$ ${receita.toFixed(2)} · Despesas R$ ${despesa.toFixed(2)}.` });
  } else {
    insights.push({ titulo: "DRE sem lançamentos no período", severidade: "media", detalhe: "Lançar receitas e despesas aumenta a precisão da análise." });
  }
