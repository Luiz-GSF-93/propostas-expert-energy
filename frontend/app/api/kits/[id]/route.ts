...
// PUT /api/kits/[id] - Atualiza kit existente e grava nova lista de itens
export async function PUT(
  ...
  try {
    ...
    // 2. Se não encontrou pelo ID, tenta atualizar ou criar por nome
    if (!data && body.name) {
    ...
    if (error) {
      console.error("[PUT /api/kits] Erro Supabase:", error);
      return NextResponse.json({ ok: false, error: error.message }, { status: 500 });
    }

    return NextResponse.json({ ok: true, data }, { status: 200 });
  } catch (err: any) {
    console.error("[PUT /api/kits] Erro fatal:", err);
